...
@section('content')
    ...
    @foreach($supplierBases as $supplierBase)
        <div id="{{'base-order-' . $supplierBase->base_type_id}}"
             class="{{'base-order-' . $supplierBase->base_type_id}} base-order">
            ...
            <form class="base-order__form" method="POST" action="{{route('supplierBaseSubmit')}}">
                @csrf
                <div class="base-order__group">
                    <label class="base-order__label">Введите ваш Email</label>
                    <input type="text" name="email" class="base-order__input" placeholder="Email">
                </div>
                <input type="hidden" name="base_type_id" value="{{ $supplierBase['base_type_id'] }}">
                <button class="base-order__btn {{'base-order-ready-open-modal-' . $supplierBase->base_type_id}}"
                        type="button">
                    ОПЛАТИТЬ
                </button>
            </form>
        ...
        <div id="{{'base-order-ready-' . $supplierBase->base_type_id}}"
             class="{{'base-order-ready-' . $supplierBase->base_type_id}} base-order base-order-ready">
            <h3 class="base-order__title">
                ВАШ ЗАКАЗ
            </h3>
            <div class="base-order-price">
