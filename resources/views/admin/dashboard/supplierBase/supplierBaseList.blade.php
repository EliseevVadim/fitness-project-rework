...
--}}
        ...
        <div class="animated fadeIn">
            <div class="row">
                <div class="col-sm-12 col-md-12">
                    <a href="/admin/supplier-bases/add" class="btn btn-primary btn-lg mb-3">Добавить</a>
                </div>
                <div class="col-lg-12">
                    <div class="card">
                        <div class="card-header"><i class="fa fa-align-justify"></i>Базы</div>
                        <div class="card-body">
                                ...
                                @isset($supplierBases)
                                    @foreach($supplierBases as $supplierBase)
                                        <tr>
                                            ...
                                            <td>{{ $supplierBase->supplierBaseTypes->name}}</td>
                                            <td>{{ $supplierBase->price}}</td>
                                            <td>{{ $supplierBase->discount_price}}</td>
                                            <td>{{ $supplierBase->content_link }}</td>
                                            <td class="btns-edit">
                                                <a href="/admin/supplier-bases/edit/{{$supplierBase->id}}" class="users-btn btn btn-block btn-primary">
                                                ...
                                                <button class="users-btn btn btn-block btn-danger" onclick="deleteSupplierBase({{$supplierBase->id}})">
    ...
    <script>
        function deleteSupplierBase(id) {
            axios.delete('/admin/supplier-bases/remove/' + id)
                .then(() => {
                    window.location.href = '/admin/supplier-bases';
                })
                .catch((error) => {
                    console.log(error);
                })
