...
@section('content')
        ...
        <div class="animated fadeIn">
            <div class="row">
                <div class="col-sm-12 col-md-12">
                    <a href="/admin/program-contents/add" class="btn btn-primary btn-lg mb-3">Добавить</a>
                </div>
                <div class="col-lg-12">
                    <div class="card">
                        <div class="card-header"><i class="fa fa-align-justify"></i>Все содержимое программ</div>
                        <div class="card-body">
                            <table class="table table-responsive-sm table-bordered table-striped table-sm">
                                ...
                                <tr>
                                    ...
                                    <th>Калории</th>
                                    <th>Ссылка на файл в google drive</th>
                                    <th>Настройки</th>
                                </tr>
                                </thead>
                                <tbody>
                                @isset($programContents)
                                    @foreach($programContents as $programContent)
                                        <tr>
                                            <td>{{ $programContent->name}}</td>
                                            <td>{{ $programContent->trainingLocation->name}}</td>
                                            <td> {{ $programContent->menuCalories->name}}</td>
                                            <td>{{ $programContent->google_drive_link}}</td>
                                            <td class="btns-edit">
                                                <a href="/admin/program-contents/edit/{{$programContent->id}}" class="users-btn btn btn-block btn-primary">
                                                ...
                                                <button class="users-btn btn btn-block btn-danger" onclick="deleteMenu({{$programContent->id}})">
    ...
    <script>
        function deleteMenu(id) {
            axios.delete('/admin/program-contents/remove/' + id)
                .then(() => {
                    window.location.href = '/admin/program-contents';
                })
                .catch((error) => {
                    console.log(error);
                })
